...
def build():
    ...
    for m, sel in (("md", "dot_s"), ("mb", "box_s"), ("mg", "bg_s")):
    ...
    # ---- fills (2D only here) ----
    n += [helper.make_node("Cast", ["mg"], ["bgb"], to=B),
          # right: col>=C1p & col+1<=rmd
          ...
          helper.make_node("Or", ["hM", "vM"], ["f0m"]),
          helper.make_node("And", ["f0m", "bgb"], ["fill"])]  # [1,1,24,24] bool
